...
public class GuestListJava8 {
	...
	public void printListOfExpectedFemaleGuests(List<Guest> invited) {
		List<String> expectedGuestList = invited.stream().filter(guest -> {
			try {
		...
		}).filter(guest -> guest.getGender() == Gender.F).map(guest -> guest.getName()).collect(Collectors.toList());
		expectedGuestList.forEach(exGuestNames -> System.out.println(exGuestNames));
	}
